    ...
    public var body: some View {
        HStack(spacing: 5) {
            ...
            if model.menuReadout != .icon {
                Text(model.menuText).font(.system(size: 11, weight: .medium, design: .monospaced)).frame(width: 66, alignment: .trailing)
            }
        }.accessibilityLabel("OMLX Scope, \(model.statusText), \(model.menuText)")
    }
}
